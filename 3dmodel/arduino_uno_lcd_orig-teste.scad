...
part = "top"; // [ top, bottom, button, buttons, all, teste ]
...
pcbmnt3dy = -(pcb1/2 - 8.0 - 3.8/2);
pcbmnt3ady = (pcb1/2 - 18.6 - 3.1/2);
...
button = [-(80.1/2-25.8+6.0/2), -(57.9/2-3.6-6.0/2), 5.3];
led_dia = 5.0;
buzzer_dia = 12.0;
button_dia = 4.6;
button_dy1 = 4.7;
...
trimpotdy = pcb1/2 - 0.9 - 4.4/2; 
trimmer_dia = 2.8; // 2.3 with no margin
trimmer = [-(pcb[0]/2-4.8-trimmer_dia/2), pcb1/2-0.7-trimmer_dia/2, 0]; // coordinates of trimmer hole
lowerpcbrx = 9.5; // room at contacts, with margin 
powersy = 9.0; // power contact width, dwg is 8.9
powerdy = -(pcb1/2 - 5.9 - 9.0/2);
usbsy = 12.3;
usbdy = (pcb1/2 - (11.7 + 12.3/2));

plugrow1 = [(pcb[0]/2-24.1+2.56/2), (pcb1/2-0.5-2.56/2)]; // coordinates D7 pin
plugrow2 = [(pcb[0]/2-2.3-34.4+2.56/2), -(pcb1/2-0.5-2.56/2)]; // coordinates RST pin
...
module bottom() {
	module snap2(ex) {
		translate([pcb[0]/2+tol+wall+ex, -snap_len/2, wall+pcb2floor+pcb[2]-frame_w/2]) 
        rotate([-90, 0, 0]) 
        cylinder(r=snap_dia/2, h=snap_len, $fs=0.3);
		translate([-snap_len/2, pcb[1]/2+tol+wall, wall+pcb2floor+pcb[2]-frame_w/2]) 
        rotate([0, 90, 0]) 
        cylinder(r=snap_dia/2, h=snap_len, $fs=0.3);
	}
...
module top() {
	...
	module add() { ///a curvatura do top é aqui
		hull () for (x = [-1, 1]) for (y = [-1, 1]) {
			...
			translate([x*(pcb[0]/2+tol+wall-corner2_r+1.5) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner2_r+1.5), pcb2roof+wall-corner2_r]) 
	...
	module sub() {
       ...
       module led_hole(dx, dy, en) {  
           translate([button[0]+dx, button[1]+dy, 0]) 
                cylinder(r = led_dia/2 + swtol, h=pcb2roof+wall+(en ? d : -breakaway), $fn=32);
       }
       module buzzer_hole(dx, dy, en) {  
           translate([button[0]+dx, button[1]+dy, 0]) 
                cylinder(r = buzzer_dia/2 + swtol, h=pcb2roof+wall+(en ? d : -breakaway), $fn=32);
       }
       module keyboard_hole(xy) {  
           frmheight = 3.0;
		...
		difference() { 
			// room for pcb
			translate([0, 0, -d]) c_cube(2*tol+pcb[0], 2*tol+pcb[1], d+pcb2roof);
			union () {
				// less lcd frame 
       ...
       // extra room 
       if (extra_room > 0) 
           translate([pcb[0]/2+tol+wall+extra_room/2-1.5, 0, -d]) {
            c_cube(extra_room, pcb[1]+2*tol, pcb2roof);
           }
       // room for battery
...
module teste() {
	...
	module add() { ///a curvatura do top é aqui
		hull () for (x = [-1, 1]) for (y = [-1, 1]) {
			translate([x*(12/2+tol), y*(15/2+tol), -1]) 
			//cylinder(r = corner_r+tol+wall, h = d, $fs=0.3); // include frame
			translate([x*(12/2+tol), y*(15/2+tol), pcb2roof+wall]) 
			sphere(r = 1, $fs=0.3);	
		}
	}
